...
- Add a protected Refresh workspaces action that rereads DSH's workspace registry and reruns the baseline scan. Usage is intentionally restricted to registered workspaces; unregistered cwds are ignored, and the UI explains that registry reads are manual to avoid per-event performance overhead.
