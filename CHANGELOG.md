...
- Build-time SVG validation now decodes XML entities before checking, rejects unquoted URI attributes, external CSS url() and @import targets (including escaped/commented forms), DOCTYPE/CDATA/processing instructions and every external-resource element, and lives in scripts/svg-guard.mjs so the tests exercise the exact same guard with adversarial fixtures.
