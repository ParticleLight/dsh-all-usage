  ...
  factory: (require) => {
    ...
    function statusRefreshKind(status, snapshot) {
      ...
      if (hasSplitRevisions(status) && hasSplitRevisions(snapshot)) {
        ...
        if (status.dataRevision !== snapshot.dataRevision || status.pricingRevision !== snapshot.pricingRevision) return 'query'
...
.uh-pricing-edit-row { display:grid; grid-template-columns:minmax(240px,1.2fr) minmax(260px,1.3fr) minmax(190px,.85fr) minmax(78px,.45fr) 32px; gap:10px; align-items:center; min-width:930px; }
...
.uh-pricing-edit-row input, .uh-pricing-edit-row select, .uh-pricing-tier-edit-row input { box-sizing:border-box; min-width:0; min-height:30px; border:1px solid var(--dsw-alias-border-l2); border-radius:6px; padding:4px 7px; background:var(--dsw-alias-bg-base); color:var(--dsw-alias-label-primary); font:inherit; font-size:11px; outline:none; }
.uh-pricing-edit-row input:focus, .uh-pricing-edit-row select:focus, .uh-pricing-tier-edit-row input:focus { border-color:var(--dsw-alias-brand-primary); }
...
`
    ...
    function UsagePage(props) {
      ...
      const pricingSummary = stats.pricing && typeof stats.pricing === 'object' ? stats.pricing : {}
      const currentPricing = pricingDetails && typeof pricingDetails === 'object' ? pricingDetails : pricingSummary
      const pricingUsedModels = pricingUsedModelsOf(currentPricing)
      ...
      const cellElements = cells.map((cell, i) => {
        ...
        let count = 0
        if (day !== undefined) {
          if (queryUsable || wsFilter === null) count = day.turns
          else {
      ...
      const pricingPanel = pricingOpen && pricingDraft !== null ? React.createElement('div', { className: 'uh-panel uh-pricing-panel uh-anim-panel' },
        ...
        React.createElement('div', { className: 'uh-pricing-section' },
          ...
          pricingDraft.mappings.length === 0 ? React.createElement('div', { className: 'uh-empty', style: { padding: '12px 0' } }, tr('选择当前模型后，再指定对应的官方模型。DSH Provider 不参与计价。', 'Select a used model, then choose its official model. The DSH provider is ignored.')) : pricingDraft.mappings.map((mapping, index) => {
            ...
            return React.createElement('div', { key: index, className: 'uh-pricing-edit-row' },
              ...
              React.createElement('select', { value: mapping.inputTokenSemantics || 'fresh', title: tr('输入上下文与可计费输入的 Token 口径', 'Token semantics for input context and billable input'), 'aria-label': tr('输入 Token 口径', 'Input token semantics'), onChange: (event) => updatePricingMapping(index, 'inputTokenSemantics', event.target.value) },
                ['fresh', 'total', 'legacy'].map((value) => React.createElement('option', { key: value, value }, pricingSemanticsLabel(value, language))),
              ),
        ...
        pricingError !== '' ? React.createElement('div', { className: 'uh-pricing-error', role: 'alert' }, pricingError === 'forbidden' ? tr('没有权限保存成本设置', 'Not allowed to save cost settings') : pricingError === 'token' ? tr('当前进程令牌不可用，请刷新看板', 'The process capability is unavailable; refresh the dashboard') : pricingError === 'sync' ? tr('models.dev 同步失败，已保留上次成功目录和未保存编辑', 'models.dev sync failed; the last good catalog and unsaved edits were kept') : pricingError === 'mapping' ? tr('模型映射无效：请选择当前模型、官方模型和有效的 Token 口径/倍率', 'Invalid model mapping: select a used model, an official model, and valid token semantics/multiplier') : pricingError === 'tier' ? tr('费率档位无效：最多 32 档；阈值必须为递增的正整数，四项费率必须完整且非负', 'Invalid rate bands: maximum 32; thresholds must be increasing positive integers and all four rates must be complete and non-negative') : pricingError === 'override' ? tr('价格覆盖无效：请选择模型并填写完整的非负基础费率', 'Invalid price override: select a model and enter all non-negative base rates') : tr('成本设置保存失败，请检查输入', 'Cost settings could not be saved; check the inputs')) : null,
      ...
      ) : pricingOpen ? React.createElement('div', { className: 'uh-panel uh-pricing-panel uh-anim-panel' },
        React.createElement('div', { className: 'uh-pricing-head' },
          React.createElement('div', { className: 'uh-title-with-icon' }, React.createElement(LineIcon, { name: 'wallet', size: 16 }), React.createElement('strong', {}, tr('成本统计设置', 'Cost Statistics'))),
          React.createElement('button', { type: 'button', className: 'uh-refresh uh-icon-button', title: tr('关闭成本设置', 'Close cost settings'), 'aria-label': tr('关闭成本设置', 'Close cost settings'), disabled: pricingSaving || pricingSyncing || pricingSyncSaving, onClick: closePricingPanel }, React.createElement(LineIcon, { name: 'close', size: 16 })),
        ),
      ...
      const hourlyTrendRows = queryUsable && queryScope !== null && queryScope.start === queryScope.end && queryResult && Array.isArray(queryResult.hourly) ? buildTrendHourlyRows(queryResult.hourly, useUtc) : []
      const trendRows = hourlyTrendRows.length > 0 ? hourlyTrendRows : buildTrendRows(queryUsable && queryResult && Array.isArray(queryResult.daily) ? queryResult.daily : activeDayRows, trendBounds, useUtc)
