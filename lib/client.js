...
`
    ...
    function UsagePage(props) {
      ...
      const pricingPanel = pricingOpen && pricingDraft !== null ? React.createElement('div', { className: 'uh-panel uh-pricing-panel uh-anim-panel' },
        ...
        React.createElement('div', { className: 'uh-pricing-note' }, tr('价格来自 models.dev 的公开目录，单位为 USD / 1M Token。已保存的非 tiered 历史成本不会因目录更新重算；旧 tiered 估算会迁移为 unsupported，同步只回填当前未计价的调用。', 'Prices come from the public models.dev catalog in USD per 1M tokens. Saved non-tiered historical costs are not recalculated; legacy tiered estimates migrate to unsupported, and sync only backfills currently unpriced calls.')),
