  ...
  factory: (require) => {
    ...
    function humanDate(date, language) {
      ...
      if (language === 'en') {
        ...
        return month + ' ' + Number(parts[2]) + ', ' + parts[0] + ' (' + week + ')'
      }
...
.uh-language { display:inline-flex; align-items:center; gap:5px; min-height:30px; padding:0 8px; border-radius:15px; background:color-mix(in srgb, var(--dsw-alias-label-primary) 7%, var(--dsw-alias-bg-layer-1)); color:var(--dsw-alias-label-secondary); }
.uh-language select { appearance:none; border:0; background:transparent; color:var(--dsw-alias-label-primary); font:inherit; font-size:12px; font-weight:600; outline:0; cursor:pointer; padding:0 2px; }
.uh-language select option { color:initial; }
    ...
    function UsagePage(props) {
      ...
      const exportCsv = () => {
        ...
        const output = [
          ...
          line([tr('导出时间', 'Exported At'), new Date().toLocaleString(language === 'en' ? 'en-US' : 'zh-CN')]),
      ...
      return React.createElement('div', { className: 'uh-page' },
        React.createElement('div', { className: 'uh-head' },
          ...
          React.createElement('div', { className: 'uh-actions' },
            ...
            React.createElement('label', { className: 'uh-language', title: tr('切换界面语言', 'Change interface language') },
              React.createElement(LineIcon, { name: 'language', size: 14 }),
              React.createElement('select', { value: language, 'aria-label': tr('界面语言', 'Interface language'), onChange: (event) => setLanguage(event.target.value) },
        ...
        isEmpty ? React.createElement('div', { className: 'uh-panel' },
          ...
          React.createElement('div', { className: 'uh-panel' },
            ...
            React.createElement('div', { className: 'uh-note', style: { marginTop: 10 } }, tr('口径：每完成一个回合点亮一次（含子代理会话）；悬停查看按工作区明细，点击工作区可筛选热力图与明细表。', 'Methodology: one cell lights up for each completed turn, including subagent sessions. Hover to view workspace details; click a workspace to filter the heatmap and detail tables.')),
          ),
          ),
