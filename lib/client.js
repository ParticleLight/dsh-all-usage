...
`
    ...
    function UsagePage(props) {
      ...
      const pricingPanel = pricingOpen && pricingDraft !== null ? React.createElement('div', { className: 'uh-panel uh-pricing-panel uh-anim-panel' },
        ...
        React.createElement('div', { className: 'uh-pricing-note' }, tr('价格来自 models.dev 的公开目录，单位为 USD / 1M Token。分层模型按本次请求的输入上下文选择价格档位；已保存成本不会因目录更新重算，同步只回填未计价调用。', 'Prices come from the public models.dev catalog in USD per 1M tokens. Context-tiered models select a rate from this request’s input context; saved costs are not recalculated on catalog updates, and sync only backfills unpriced calls.')),
