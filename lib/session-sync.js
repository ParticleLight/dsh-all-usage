...
import { contextTimeKey, extractUsageEvent, isUnregisteredWorkspaceId, pickPricingTime, touchContextTimes, unregisteredWorkspaceId, unregisteredWorkspaceMeta } from './usage-core.js'
...
export function createSessionSync(host) {
  ...
  function workspaceForCwd(cwd) {
    ...
    const registered = state.pathIndex.get(cwd)
    if (registered !== undefined) return registered
    // Deleted historical directories must not become synthetic workspaces. The
    // cache avoids a filesystem stat on every session/event; manual Refresh
    // workspaces clears it before the next baseline.
    let exists = workspaceDirectoryCache.get(cwd)
    if (exists === undefined) {
      try { exists = statSync(cwd).isDirectory() } catch (_) { exists = false }
      workspaceDirectoryCache.set(cwd, exists)
    }
    if (!exists) return undefined
    const id = unregisteredWorkspaceId(cwd)
    if (id === undefined) return undefined
    if (!state.wsMeta.has(id)) {
      const meta = unregisteredWorkspaceMeta(id)
      if (meta !== undefined) {
        state.wsMeta.set(id, meta)
  ...
  async function runBaseline(generation = state.aggregationGeneration) {
    ...
    for (const [sid, record] of state.ledgerRecords) {
      if (!listedSessionIds.has(sid)) {
        ...
        if (isUnregisteredWorkspaceId(record.workspaceId) && record.sourceCwd === undefined) continue
  ...
  async function refreshWorkspaceRegistry() {
    if (state.disposed) return { ok: false, message: 'disposed' }
