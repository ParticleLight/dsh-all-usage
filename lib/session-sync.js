import { RETIRED_WORKSPACE_ID, retargetLedgerRecord } from './ledger.js'
...
// The registry event is a fast path, not a guarantee: a change that lands while a
// baseline runs used to be dropped outright, and a missed event left the plugin
// stale until the next restart. The (in-memory) registry list is polled as well.
const WORKSPACE_PROBE_INTERVAL_MS = 30000
...
export function createSessionSync(host) {
  ...
  // DSH 0.1.5 replaced \`sessionPersistence.listSnapshots()\` with \`list()\`; both
  // return per-session \`{ header, revision }\` rows. Without a revision the
  // ledger-reuse fast path never fires, so every baseline re-reads every log.
  async function loadSessionRevisions() {
    const source = sessionPersistence
    if (source === undefined || source === null) return null
    const read = typeof source.list === 'function'
      ? () => source.list()
      : (typeof source.listSnapshots === 'function' ? () => source.listSnapshots() : null)
    ...
    for (const row of rows) {
      ...
      if (rid !== undefined && row && typeof row.revision === 'string') revisions.set(rid, row.revision)
    }
    return revisions
  }
  ...
  async function runBaseline(generation = state.aggregationGeneration) {
    ...
    let snapshots = null
    try {
      const revisions = await loadSessionRevisions()
      if (state.disposed || generation !== state.aggregationGeneration) return
      if (revisions !== null) {
    ...
    for (const [sid, record] of state.ledgerRecords) {
      if (!listedSessionIds.has(sid)) {
        // Recorded usage survives deletion. A session whose workspace, cwd or
        // log is gone still counts from its ledger row; only legacy synthetic
        // rows, which never belonged to a registered workspace, stay excluded.
        if (typeof record.workspaceId === 'string' && record.workspaceId.startsWith('unregistered:')) continue
        // Usage from a workspace that is no longer registered is summed into the
        // shared deleted bucket, so history survives without a row per dead
        // workspace. The stored row keeps its original workspace id and cwd.
        const retired = typeof record.workspaceId !== 'string' || !state.wsMeta.has(record.workspaceId)
        const carriesUsage = record.turns.length > 0 || record.usage.length > 0
        // An empty row must not advertise a bucket that holds nothing.
        if (retired && carriesUsage) state.retiredBucketUsed = true
        applyLedgerRecord(retired ? retargetLedgerRecord(record) : record, retired ? 'ledger-retired' : 'ledger-recovery')
        if (carriesUsage) state.sync.sessionsRestoredFromLedger += 1
      }
    }
    ...
    if (state.workspaceSyncDeferred) {
      state.workspaceSyncDeferred = false
  ...
  async function runWorkspaceSync() {
    ...
    state.sync.workspaceSyncRuns += 1
    try {
  ...
  async function synchronizeWorkspaceRegistry() {
    if (state.disposed) return
    // A registry change that lands while a baseline is running used to be
    // dropped outright, which is why a new workspace could stay invisible until
    // the next restart. Remember it and re-run once the scan settles.
    if (!state.scan.done) {
      state.workspaceSyncDeferred = true
      state.sync.workspaceSyncDeferred += 1
      return
    }
    let workspaces
    try {
    ...
    state.sync.workspaceSyncAdded += addedWs.size
    state.sync.workspaceSyncRemoved += removedWs.size
    // Reuse path: no membership change — metadata only, zero rescan.
    if (removedWs.size === 0 && addedWs.size === 0) {
      installWorkspaceSnapshot(next)
      return
    }
    // Retire path: a deregistered workspace keeps everything it recorded. The
    // ledger's promise is that flushed usage survives deletion, so sessions are
    // re-applied under the shared deleted bucket instead of being subtracted and
    // dropped. New sessions still cannot enter the workspace: pathIndex only
    // ever carries registered workspaces, so workspaceForCwd() no longer
    // resolves its cwd and the scan skips it.
    if (removedWs.size > 0) {
      const retiredSids = new Set()
      for (const [sid, record] of state.ledgerRecords) {
        if (record === null || record === undefined || !removedWs.has(record.workspaceId)) continue
        retiredSids.add(sid)
      }
      for (const [sid, id] of state.memberOf) if (removedWs.has(id)) retiredSids.add(sid)
      let moved = 0
      for (const sid of retiredSids) {
        if (typeof sid !== 'string' || sid === '') continue
        // Re-key the live aggregate first: usage folded from the feed but not yet
        // persisted has no ledger row to replay, and a session the registry no
        // longer lists is never re-scanned afterwards.
        moved += host.aggregation.retargetSession(sid, RETIRED_WORKSPACE_ID)
        const record = state.ledgerRecords.get(sid)
        // Keep the in-memory record aligned with the aggregate; the persisted row
        // keeps its original workspace id and cwd, so recovery re-derives this.
        if (record !== undefined && (record.turns.length > 0 || record.usage.length > 0)) moved += 1
        if (record !== undefined) state.ledgerRecords.set(sid, retargetLedgerRecord(record))
      }
      if (moved > 0) state.retiredBucketUsed = true
    }
    installWorkspaceSnapshot(next)
    ...
    let snapshots = null
    try {
      snapshots = await loadSessionRevisions()
    } catch (err) {
      console.error('[all-usage] session persistence snapshots unavailable:', err)
    }
    for (const record of records) {
  ...
  // Belt and braces for the registry probe: the event fast path can be missed
  // (dropped while scanning, or never delivered), and unlike the event this
  // cannot silently leave the workspace list stale until a restart.
  function scheduleWorkspaceProbeTimer() {
    if (state.disposed || state.workspaceProbeTimer !== null) return
    state.workspaceProbeTimer = setTimeout(() => {
      state.workspaceProbeTimer = null
      if (!state.disposed) {
        void runWorkspaceSync()
        scheduleWorkspaceProbeTimer()
      }
    }, WORKSPACE_PROBE_INTERVAL_MS)
    if (state.workspaceProbeTimer && typeof state.workspaceProbeTimer.unref === 'function') state.workspaceProbeTimer.unref()
  }
  ...
  ctx.on('domain/changed', (change) => {
    ...
    state.sync.workspaceProbeEvents += 1
    scheduleWorkspaceSync()
  })
