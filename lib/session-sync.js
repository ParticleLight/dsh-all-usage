...
export function createSessionSync(host) {
  ...
  async function runBaseline(generation = state.aggregationGeneration) {
    ...
    try {
      ...
      for (const w of workspaces) {
        ...
        state.wsMeta.set(id, { id, title, path })
        if (path !== '') state.pathIndex.set(path, id)
        if (w && Array.isArray(w.sessionIds)) {
          for (const sid of w.sessionIds) state.memberOf.set(sid, id)
        }
      }
    } catch (err) {
    ...
    if (state.sync.sessionsRestoredFromLedger > 0) markStatsChanged()
    for (const record of records) {
      if (state.disposed || generation !== state.aggregationGeneration) return
      ...
      await enqueue(sid, async () => {
        ...
        try {
          ...
          if (!state.liveResyncPending.has(sid) && previousRecord !== undefined && previousRecord.needsUpgrade !== true && typeof previousRecord.lastRevision === 'string' && typeof revision === 'string' && revision === previousRecord.lastRevision) {
  ...
  ctx.on('session/flush', async (session) => {
    ...
    if (wsId === undefined) return
    const ledger = buildLedgerRecord(session, wsId, 'flush', undefined, state.ledgerRecords.get(session.id))
