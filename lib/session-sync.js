...
export function createSessionSync(host) {
    ...
    for (const record of records) {
      ...
      await enqueue(sid, async () => {
        ...
        try {
          ...
          if (!state.liveResyncPending.has(sid) && previousRecord !== undefined && previousRecord.needsUpgrade !== true && previousRecord.workspaceId === wsId && typeof previousRecord.lastRevision === 'string' && typeof revision === 'string' && revision === previousRecord.lastRevision) {
          ...
          if (snap && Array.isArray(snap.events)) {
            ...
            const canFoldTail = previous !== undefined && previous.needsUpgrade !== true && previous.workspaceId === wsId && !sequence.nonMonotonic && previous.lastSeq >= 0 && currentLastSeq > previous.lastSeq
            if (canFoldTail) {
  ...
  ctx.on('session/flush', async (session) => {
    ...
    const currentLastSeq = tailEvent !== null && typeof tailEvent.seq === 'number' && tailEvent.seq >= 0 ? tailEvent.seq : lastSeqOf(eventList)
