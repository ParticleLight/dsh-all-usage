...
export function createSessionSync(host) {
  ...
  ctx.on('session/flush', async (session) => {
    ...
    const wsId = wsForLiveSession(session, sid)
    if (wsId === undefined) return
    // A flush whose tail does not advance can contain rewritten history: refuse
    // to fold it and ask for a full baseline rebuild instead, so the in-memory
    // index and the persisted ledger cannot disagree about invalid sequences.
    if (previous !== undefined && currentLastSeq <= previous.lastSeq) {
