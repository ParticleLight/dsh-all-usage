  ...
  async function syncLiveSession(sid, wsId, event, generation) {
    try {
      ...
      if (snap && Array.isArray(snap.events)) {
        ...
        // A snapshot that already contains the triggering event (or a timer
        // retry) is authoritative: the history was read completely, so the
        // session is rebuilt atomically and the cursor follows the snapshot
        // exactly (rewritten/truncated history must not keep an old cursor).
        // A snapshot that still lags behind the event is only a partial read:
        // old records are kept (upsert semantics) and a follow-up resync will
        // align when the full history arrives.
        const authoritative = event === null || event === undefined || eventSeq < 0 || eventSeq <= snapshotLast
        if (authoritative) host.aggregation.removeSession(sid)
        foldEvents(wsId, snap.events, undefined, sid, 'live')
