import { contextTimeKey, extractUsageEvent, pickPricingTime } from './usage-core.js'
...
export function createSessionSync(host) {
  ...
  function foldEvent(wsId, time, type, data, sid, seq, materialization = 'live') {
    ...
    if (type === 'request/context') {
      ...
      if (validEventTime(time)) {
        ...
        times.set(contextTimeKey(data && data.turn, data && data.step), time)
      }
  ...
  function foldEvents(wsId, events, fromSeq, sid, materialization = 'scan') {
    // A full fold rewrites the session from scratch: stale request-context
    // instants and route identity from an earlier read must not leak into the
    // rebuilt view, because a history rewrite can remove the very events that
    // set them. Incremental folds keep both as seeds for the new tail.
  ...
  async function syncLiveSession(sid, wsId, event, generation) {
    try {
      ...
      if (snap && Array.isArray(snap.events)) {
        ...
        state.sessionCount.add(sid)
        // A successful full sync absorbs the pending event (realign or fallback
        // below): drop the flag so later live events do not re-trigger a full
        // re-read and rebuild the session's identity and context from scratch.
        state.liveResyncPending.delete(sid)
