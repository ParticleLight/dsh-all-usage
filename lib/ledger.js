...
function pollutedLedgerKey(key) {
  // Older builds stored raw sequences (including NaN, Infinity and values past
  // Number.MAX_SAFE_INTEGER) in event keys; any such key needs a rebuild.
  if (typeof key !== 'string') return true
  if (key.includes('Infinity') || key.includes('NaN')) return true
  if (/^-\d+$/.test(key)) return true
  if (/^\d+$/.test(key)) return !Number.isSafeInteger(Number(key))
  return false
}
...
export function createLedger(host) {
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    const computedReason = invalidTurn ? 'invalid-turn' : invalidUsage ? 'invalid-usage' : mixedWorkspace ? 'mixed-workspace' : (raw.lastSeq !== undefined && !validLedgerSeq(raw.lastSeq)) ? 'invalid-last-seq' : undefined
