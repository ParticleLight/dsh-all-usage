...
import { billingInstantOf, contextTimeKey, extractUsageEvent, normalizeUsageValues as usageValues, pickPricingTime, upsertUsageSample as upsertUsageSampleState, usageStepKey } from './usage-core.js'
  ...
  function buildLedgerRecord(session, workspaceId, source = 'scan', revision, previousRecord, sequenceClean = true) {
    ...
    for (let index = startIndex; index < events.length; index += 1) {
      ...
      if (event.type === 'request/context') {
        ...
        if (validEventTime(event.time)) contextTimes.set(contextTimeKey(data && data.turn, data && data.step), event.time)
        continue
      }
    ...
    while (contextTimes.size > MAX_CONTEXT_TIMES) contextTimes.delete(contextTimes.keys().next().value)
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    if (Array.isArray(raw.contextTimes)) {
      for (const entry of raw.contextTimes) {
        ...
        contextMap.set(entry.key, entry.time)
      }
    }
