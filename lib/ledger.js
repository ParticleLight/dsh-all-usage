...
export function createLedger(host) {
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    const invalidTurn = raw.turns.some((turn) => turn === null || typeof turn !== 'object' || typeof turn.key !== 'string' || turn.workspaceId === undefined || !validEventTime(turn.time) || (turn.seq !== undefined && !Number.isSafeInteger(turn.seq)))
    const invalidUsage = raw.usage.some((item) => { const cost = normalizeCostSnapshot(item && item.cost); return item === null || typeof item !== 'object' || !validEventTime(item.time) || item.identity === undefined || (item.seq !== undefined && !Number.isSafeInteger(item.seq)) || cost === null || cost.pricingMode !== 'official-model' || isLegacyTieredCost(cost) })
    ...
    const needsUpgrade = !Number.isFinite(raw.updatedAt) || raw.version !== LEDGER_VERSION || invalidTurn || invalidUsage || mixedWorkspace || (raw.lastSeq !== undefined && !Number.isSafeInteger(raw.lastSeq))
  ...
  function prepareLedgerRecord(record) {
    ...
    // Cost repair does not fix a record whose historical items belong to a
    // different workspace; keep the upgrade flag until a rebuild normalizes it.
