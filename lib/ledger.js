...
export function createLedger(host) {
  ...
  function canFoldLedgerTail(previousRecord, events) {
  ...
  function buildLedgerRecord(session, workspaceId, source = 'scan', revision, previousRecord) {
    ...
    const canFoldTail = canFoldLedgerTail(previousRecord, events)
  ...
  function scheduleLedgerWrite() {
    ...
    if (state.ledgerWriteTimer && typeof state.ledgerWriteTimer.unref === 'function') state.ledgerWriteTimer.unref()
  }
