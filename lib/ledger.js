...
function pollutedLedgerKey(key) {
  // Older builds stored raw String(seq) in event keys. A bare numeric text is a
  // sequence key and is only valid as canonical non-negative safe-integer
  // decimal text; anything else (float/exponent/negative/huge) needs a rebuild.
  // Composite keys (usageStepKey, positional event:N) are untouched.
