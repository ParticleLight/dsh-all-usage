...
function pollutedLedgerKey(key) {
  // Older builds stored raw sequences (and NaN/Infinity) in event keys; any
  // key carrying such a value, or a negative sequence string, needs a rebuild.
  if (typeof key !== 'string') return true
  return key.includes('Infinity') || key.includes('NaN') || /^-\d+$/.test(key)
}
...
export function createLedger(host) {
  ...
  function canFoldLedgerTail(previousRecord, events, workspaceId) {
    if (previousRecord === null || previousRecord === undefined || previousRecord.needsUpgrade === true || !Number.isSafeInteger(previousRecord.lastSeq) || previousRecord.lastSeq < 0 || !Array.isArray(events) || events.length === 0) return false
  ...
  function buildLedgerRecord(session, workspaceId, source = 'scan', revision, previousRecord) {
    ...
    for (let index = startIndex; index < events.length; index += 1) {
      ...
      if (event.type === 'turn/end') {
        ...
        turns.set(key, { key, seq: Number.isSafeInteger(event.seq) ? event.seq : -1, time: event.time, workspaceId, turn, identity: currentIdentity })
        continue
      }
      if (!validEventTime(event.time)) continue
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    const needsUpgrade = !Number.isFinite(raw.updatedAt) || raw.version !== LEDGER_VERSION || invalidTurn || invalidUsage || mixedWorkspace || (raw.lastSeq !== undefined && !validLedgerSeq(raw.lastSeq))
