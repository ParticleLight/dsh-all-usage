...
export function createLedger(host) {
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    const needsUpgrade = !Number.isFinite(raw.updatedAt) || raw.version !== LEDGER_VERSION || raw.usage.some((item) => { const cost = normalizeCostSnapshot(item && item.cost); return item === null || typeof item !== 'object' || item.identity === undefined || cost === null || cost.pricingMode !== 'official-model' })
