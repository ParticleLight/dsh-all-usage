...
export function createLedger(host) {
  ...
  function normalizeLedgerRecord(raw, key) {
    ...
    const invalidUsage = raw.usage.some((item) => { const cost = normalizeCostSnapshot(item && item.cost); return item === null || typeof item !== 'object' || !validEventTime(item.time) || item.identity === undefined || cost === null || cost.pricingMode !== 'official-model' || cost.tiered === true })
  ...
  function prepareLedgerRecord(record) {
    ...
    for (const item of record.usage) {
      ...
      if (cost !== null && cost.pricingMode === 'official-model' && cost.tiered !== true) { item.cost = cost; continue }
      if (cost !== null && cost.tiered === true) {
