...
function pollutedLedgerKey(key) {
  // Older builds stored raw String(seq) in event keys. Any bare numeric text
  // (leading zeros, signs, whitespace, decimals, exponents, huge values) is a
  // non-canonical sequence key and needs a rebuild; composite keys that start
  // with a letter (usageStepKey, positional event:N) are untouched.
