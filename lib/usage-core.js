...
export const UNREGISTERED_WORKSPACE_PREFIX = 'unregistered:'
export const UNREGISTERED_WORKSPACE_TITLE = '未注册工作区'

function normalizedWorkspaceCwd(cwd) {
  if (typeof cwd !== 'string') return ''
  return cwd.trim().replaceAll('\\', '/').replace(/\/+$/, '').toLowerCase()
}

/** Stable, privacy-preserving workspace bucket for a cwd absent from registry. */
export function unregisteredWorkspaceId(cwd) {
  const normalized = normalizedWorkspaceCwd(cwd)
  if (normalized === '') return undefined
  return UNREGISTERED_WORKSPACE_PREFIX + createHash('sha256').update(normalized).digest('hex').slice(0, 16)
}

export function isUnregisteredWorkspaceId(value) {
  return typeof value === 'string' && value.startsWith(UNREGISTERED_WORKSPACE_PREFIX)
}

export function unregisteredWorkspaceMeta(id) {
  if (!isUnregisteredWorkspaceId(id)) return undefined
  return { id, title: UNREGISTERED_WORKSPACE_TITLE + ' · ' + id.slice(UNREGISTERED_WORKSPACE_PREFIX.length, UNREGISTERED_WORKSPACE_PREFIX.length + 8), path: '' }
}
