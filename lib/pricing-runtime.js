...
export function createPricingRuntime(host) {
  ...
  function pricingSnapshot() {
    ...
    return {
      ...
      config: { sync: { ...pricingStateSnapshot.sync }, mappings: pricingStateSnapshot.mappings.map((mapping) => ({ ...mapping })), overrides: pricingStateSnapshot.overrides.map(({ providerId, ...entry }) => ({ ...entry })) },
      usedModels: used,
