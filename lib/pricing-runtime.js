...
export function createPricingRuntime(host) {
  ...
  function usedPricingModels(tierSchedules = null) {
    ...
    for (const item of state.usageByStep.values()) {
      ...
      rows.push({
        ...
        ...(resolved.temporalProfile && resolved.temporalProfile.policyId ? { temporalPolicyId: resolved.temporalProfile.policyId, temporalTimezone: resolved.temporalProfile.timezone === 'UTC' ? 'UTC' : null } : {}),
      })
  ...
  /**
   * Controlled DeepSeek temporal reconciliation: v1 snapshots of first-party
   * DeepSeek routes are re-priced against the usage instant, v2 snapshots whose
   * band/policy no longer match the current plan are re-computed, and anything
   ...
   */
  function reconcileTemporalPricing() {
    let considered = 0
    let reconciled = 0
    const touched = new Set()
    for (const item of state.usageByStep.values()) {
