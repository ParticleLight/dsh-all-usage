...
function attachTemporalProfile(entry, mapped, identity) {
  ...
  if (profile === null && normalizeProvider(entry.providerId) === 'deepseek') profile = builtinTemporalProfileFor(entry.modelId)
  ...
  const route = providerId !== '' && identityProvider === providerId ? 'official' : mapped !== undefined && mapped !== null ? 'mapped' : 'other'
  return {
    route,
    profile: {
      ...profile,
