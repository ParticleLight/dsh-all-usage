...
function parseModelsDevCatalog(raw, fetchedAt = Date.now()) {
  ...
  // The fetchedAt stamp differs on every fetch and upstream object order is
  // not canonical; exclude the stamp and sort stably so semantically identical
  // catalogs hash identically across syncs.
  const canonical = JSON.stringify(entries.map((entry) => {
    if (entry.fetchedAt === undefined) return entry
    const { fetchedAt, ...stable } = entry
    return stable
  }).sort((left, right) => priceEntryKey(left).localeCompare(priceEntryKey(right))))
  return {
    ok: true,
    catalog: {
