...
function parseModelsDevCatalog(raw, fetchedAt = Date.now()) {
  ...
  // Sort by stable key BEFORE truncating: upstream object order must not decide
  // which models survive a large catalog, or identical contents would hash
  // differently (and drop different models) across fetches.
  entries.sort((left, right) => priceEntryKey(left).localeCompare(priceEntryKey(right)))
  const ordered = entries.slice(0, MAX_PRICE_ENTRIES)
