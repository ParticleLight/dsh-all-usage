...
import { RETIRED_WORKSPACE_ID, createLedger } from './ledger.js'
...
function apply(ctx) {
  ...
  const state = {
    wsMeta: new Map(),
    // True once recorded usage from a vanished workspace was moved into the
    // shared "deleted" bucket; the bucket row is only advertised when it holds
    // something.
    retiredBucketUsed: false,
    // Last client-side environment report (diagnostics only): the desktop host
    // cannot be inspected from outside the app.
    clientEnv: null,
    pathIndex: new Map(),
    memberOf: new Map(),
    byDay: new Map(),
    ...
    sync: {
      ...
      sessionsFailed: 0,
      // Cumulative registry-probe diagnostics. They deliberately survive a
      // baseline reset so the status endpoint can show whether the
      // `domain/changed` fast path fires at all on this host.
      workspaceProbeEvents: 0,
      workspaceSyncRuns: 0,
      workspaceSyncAdded: 0,
      workspaceSyncRemoved: 0,
      workspaceSyncDeferred: 0,
    },
    ...
    workspaceSyncDeferred: false,
    workspaceProbeTimer: null,
  ...
  function syncSnapshot() {
    return {
      ...
      workspaceProbeEvents: state.sync.workspaceProbeEvents,
      workspaceSyncRuns: state.sync.workspaceSyncRuns,
      workspaceSyncAdded: state.sync.workspaceSyncAdded,
      workspaceSyncRemoved: state.sync.workspaceSyncRemoved,
      workspaceSyncDeferred: state.sync.workspaceSyncDeferred,
    }
  }
  ...
  function setAlias(wsId, raw) {
    ...
    if (!state.wsMeta.has(wsId) && !(state.retiredBucketUsed && wsId === RETIRED_WORKSPACE_ID)) return { ok: false, message: 'unknown-workspace', aliases: Object.assign({}, state.aliases) }
  ...
  host.sessionSync.scheduleWorkspaceProbeTimer()
