...
import { createEmptyPricingState, emptyCostAggregate } from './pricing.js'
...
function apply(ctx) {
  ...
  const state = {
    ...
    turnsByLocalDate: new Map(),
    turnsByUtcDate: new Map(),
    sessionModel: new Map(),
    totals: { turns: 0, input: 0, output: 0, cacheRead: 0, cacheWrite: 0, reasoning: 0, cost: emptyCostAggregate() },
    ...
    try {
      ...
      if (global !== null && global !== undefined && typeof global === 'object') {
        for (const key of Object.keys(global)) {
          ...
          if (typeof value === 'string' && value.trim() !== '') state.aliases[key] = value
        }
        markStatsChanged()
      }
    } catch (err) {
  ...
  ctx.effect(() => async () => {
    ...
    state.chains.clear()
    await Promise.all([state.aliasesReady, state.ledgerReady, state.pricingReady, state.aliasWriteChain, state.ledgerWriteChain, state.pricingWriteChain])
