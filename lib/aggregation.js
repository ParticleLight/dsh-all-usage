...
export function createAggregation(host) {
  ...
  function costForUsage(values, identity, previous, pricingAt, pricingTimeSource) {
    ...
    if (previousCost !== null && previousCost.pricingMode === 'official-model' && previousCost.pricingTimeSource !== 'legacy-unknown' && usageBasisEqual(previous, identity, values) && costTemporalCompatible(previousCost, identity, pricingAt)) return previousCost
    return calculateCost(values, resolveCurrentPricing(identity), pricingAt, pricingTimeSource)
  }
  function costTemporalCompatible(previousCost, identity, pricingAt) {
    ...
    return previousCost.pricingBand === plan.band && previousCost.pricingPolicyId === plan.policyId && previousCost.pricingPolicyHash === plan.policyHash && previousCost.pricingTimezone === plan.timezone && previousCost.temporalApplicable === (plan.status === 'applied') && previousCost.temporalExemptReason === plan.exemptReason
  }
