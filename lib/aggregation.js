...
import { extractUsageEvent, normalizeEventSeq, normalizeUsageValues as usageValues, unregisteredWorkspaceMeta, upsertUsageSample as upsertUsageSampleState, usageStepKey, validEventTime as validUsageEventTime } from './usage-core.js'
...
export function createAggregation(host) {
  ...
  function ensureWs(wsId) {
    ...
    if (ws === undefined) {
      ...
      const synthetic = unregisteredWorkspaceMeta(wsId)
      if (synthetic !== undefined && !state.wsMeta.has(wsId)) state.wsMeta.set(wsId, synthetic)
      markStatsChanged('metadata')
    }
    return ws
  }
