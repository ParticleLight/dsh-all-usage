...
async function runRuntimeSmoke(runtime) {
  ...
  try {
    ...
    assert.ok(exactBeforeDispose >= pluginPaths.length)
    await root.loader.remove(allUsageId)
    await root.loader.await()
    assert.equal(webServer.exact.size, exactBeforeDispose - pluginPaths.length)
    for (const path of pluginPaths) assert.equal(webServer.exact.has(path), false, 'dispose must remove ' + path)
