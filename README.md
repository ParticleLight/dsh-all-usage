...
- **工作区边界**：能按 cwd 映射到已注册工作区的会话使用原工作区；无法映射的会话归入稳定的“未注册工作区”桶（客户端不暴露完整 cwd）；会话尚未成功 flush 前删除或损坏的日志无法由独立账本恢复。
...
- 本地统计包含已注册工作区和稳定的未注册工作区桶，并可能因日志损坏、清理或上游没有发出 usage 而少于官方账单。
...
- **Workspace boundary**: sessions whose cwd maps to a registered workspace keep that workspace; otherwise they use a stable unregistered-workspace bucket without exposing the full cwd to the client. Data deleted or corrupted before a successful session flush cannot be recovered from the separate ledger.
...
- Local statistics include registered workspaces and stable unregistered-workspace buckets, and can be lower than the official bill when logs are damaged, cleaned up, or the upstream emits no usage event.
